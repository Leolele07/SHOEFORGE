...
- **智能图层识别**：自动跳过Sketchfab_model、Collada visual scene group等容器节点，精准定位图层根
- **完整图层映射**：补全15个犀牛图层名的中英文名称映射（Midsole/中底、Upper/鞋面、Outsole/外底等）
- **隐藏部件修复**：隐藏部件后眼睛按钮仍可正常点击，行会变灰但按钮清晰可见
- **开闭眼图标优化**：隐藏状态显示闭眼+斜杠图标，显示状态显示睁开眼睛图标
