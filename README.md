...
- **材质定制** - 11 种 PBR 材质（皮革/网面/麂皮/帆布/光面/金属/塑料/橡胶/布料/碳纤维/透明）
...
### v1.0.9 (2026-07-29) - 原始材质保留与恢复
- **核心改进**：上传模型后保留原始材质外观（颜色、贴图、roughness、metalness）
- **智能覆盖**：只覆盖用户主动修改的部件，未修改部件保留原始外观
- **恢复原始**：支持一键恢复到模型导入时的原始状态
- **性能优化**：不转base64，直接保留原始材质引用，性能好
- **已知问题**：一键白膜功能暂未生效，将在后续版本修复

### v1.0.7 (2026-07-28) - 白模模式
- **核心改进**：加载模型后自动显示为白色默认材质
- **设计优化**：移除原始材质和颜色，保留几何形状和结构细节
- **细节保留**：保留法线贴图等细节，确保模型外观完整
- **用户体验**：方便用户从零开始设计，无需手动清除原始材质

### v1.0.6 (2026-07-28) - 贴图系统
- **核心功能**：支持上传贴图并调整位置、大小、旋转
- **贴图类型**：支持颜色贴图、法线贴图、粗糙度贴图、金属度贴图
- **变换控制**：可调整贴图的X/Y偏移、X/Y缩放、旋转角度
- **简化材质**：移除程序化纹理，只保留基本PBR参数
- **UI改进**：新增贴图标签页，提供直观的贴图管理界面
